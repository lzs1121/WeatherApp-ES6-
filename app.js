...
const APP_ID = 'ad8c25278c454ab78c28790890f6747';

ELEMENTS: ELEMENTS.ELEMENT_SEARCH_BUTTON.addEventListener('click', searchWeather);
...
function searchWeather() {
    ...
    Http.fetchData(URL)
        .then(responseData => {
            const WEATHER_DATA = WeatherData(CITY_NAME, responseData.weather[0].description.toUpperCase());
            const WEATHER_PROXY = new Proxy(WEATHER_DATA, WEATHER_PROXY_HANDLER);
            WEATHER_PROXY.temperature = responseData.main.temp;
        })
        .catch(errpr => alert(error));
}
